...
src/avalanche/__init__.py version: <package-version>
CHANGELOG.md heading: <changelog-version>
A release commit containing only those three files will be created, then tagged and pushed to origin.
Proceed?
```
...
1. Change `[project].version` in `pyproject.toml` to the normalized package
   version.
2. Change `__version__` in `src/avalanche/__init__.py` to the normalized package
   version.
3. Move the entire non-empty body immediately below `## Unreleased` in
...
```bash
git diff --check
test "$(git diff --name-only)" = $'CHANGELOG.md\npyproject.toml\nsrc/avalanche/__init__.py' \
  || { echo "Only CHANGELOG.md, pyproject.toml, and src/avalanche/__init__.py may change." >&2; exit 1; }
...
grep -Fx "__version__ = \"<package-version>\"" src/avalanche/__init__.py >/dev/null \
  || { echo "src/avalanche/__init__.py version did not match the approved release." >&2; exit 1; }
...
git add pyproject.toml src/avalanche/__init__.py CHANGELOG.md
...
git show "<tag>:src/avalanche/__init__.py" | grep -Fx '__version__ = "<package-version>"'
